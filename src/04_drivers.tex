...
the driver wishes to initiate polling, a call to the \fnc{usb_polling_start}
function will create a separate fibril, which will spin on the pipe and call
polling handlers once data (or errors) arrive.

When the device driver wishes to cease polling, a call to the
\fnc{usb_polling_join} function will spontaneously wake up the polling fibril
...
over it. That prohibits any other calls to \fnc{usb_pipe_read} or other relevant
...
once \fnc{usb_polling_join} is called, the pipe is left closed for all
